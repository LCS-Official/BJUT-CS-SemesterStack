//必须实现的CP0寄存器包括：SR、CAUSE、EPC、PrID。

module cp0(pcout, rt_din, HWInt, Sel, cp0wr, EXLSet, EXLClr, 
            clk, rst, IntReq, epc, cp0dout);
  input [31:0] pcout;//中断时 pc+4存入epc 使得中断返回
  input [31:0] rt_din; //从rt的输入 mtc0
  input [5:0] HWInt; //表示六个设备的中断 从bridge
  input [4:0] Sel; //选择内部寄存器 mfco 哪一个设备输出 mtco 数据写入哪一个寄存器
  
  input cp0wr; //cp0写使能 只有mtco 允许
  input EXLSet, EXLClr;//exl的置位 清除 进入中断处理s10状态 + eret恢复时
  input clk, rst;

  output IntReq;// 是否中断
  output [31:0] epc;// 保存中断/异常时的PC+4 使得中断结束后程序返回
  output [31:0] cp0dout;  // pc0写给rt的 mfco指令
  
 
  //4个寄存器编号 参数 清晰一些
  parameter [4:0] SR = 5'd12;//状态寄存器
