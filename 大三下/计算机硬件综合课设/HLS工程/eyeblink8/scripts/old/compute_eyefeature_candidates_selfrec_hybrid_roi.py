"""
Hybrid test for PL EyeFeature candidates:
- dlib detector / landmark predictor runs on a downscaled frame.
- Eye ROI coordinates are mapped back to the original frame.
- EyeFeature candidates, especially col_run_topk_fixed, are computed on the original-resolution grayscale frame.

This simulates the intended board-side split: PS uses low-resolution frames for the expensive dlib step, while PL computes the cheap ROI statistics on the higher-resolution/original frame.
...
Outputs:
  work_selfrec_candidates_hybrid_half_lowfreq/frame_eyefeature_candidates.csv
  work_selfrec_candidates_hybrid_half_lowfreq/feature_summary_candidates.csv
  work_selfrec_candidates_hybrid_half_lowfreq/per_video_feature_summary.csv
  work_selfrec_candidates_hybrid_half_lowfreq/candidate_plots/<video_id>_candidates.png
...
def resize_frame_for_processing(frame: np.ndarray, args) -> Tuple[np.ndarray, float, float]:
    """
    Resize the whole frame before dlib ROI detection only.

    Returns:
      proc_frame, sx, sy
    where sx = proc_width / original_width, sy = proc_height / original_height.

    In this hybrid script, dlib ROI coordinates are produced in processed-frame
    coordinates and then mapped back to original-frame coordinates. EyeFeature
    is computed on the original frame.
    """
    h, w = frame.shape[:2]
...
def map_roi_to_original(roi: Optional[ROI], sx: float, sy: float, orig_w: int, orig_h: int) -> Optional[ROI]:
    """Map a processed-frame ROI back to original-frame coordinates.

    Use floor for the left/top and ceil for the right/bottom so the mapped ROI
    does not accidentally crop away eye pixels. This is the coordinate set that
    would be written from PS to the PL EyeFeature IP.
    """
    if roi is None:
        return None
    if sx <= 0 or sy <= 0:
        return None
    x, y, w, h = roi
    x0 = int(math.floor(x / sx))
    y0 = int(math.floor(y / sy))
    x1 = int(math.ceil((x + w) / sx))
    y1 = int(math.ceil((y + h) / sy))
...
def process_video(video_path: Path, args, detector, predictor, intervals_df) -> pd.DataFrame:
    ...
    last_left_proc = None; last_right_proc = None
    ...
    for frame_idx in iterator:
        ...
        # Original grayscale is the image used for EyeFeature, simulating PL-side
        # statistics on the original-resolution frame.
        orig_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Processed grayscale is used only by the expensive dlib detector / predictor.
        ...
        if need_update:
            left_p, right_p, face_ok = detect_eye_rois(proc_gray, detector, predictor, args.detect_width, args.upsample, args.pad_ratio)
            if face_ok:
                left_o = map_roi_to_original(left_p, sx, sy, orig_frame_w, orig_frame_h)
                right_o = map_roi_to_original(right_p, sx, sy, orig_frame_w, orig_frame_h)
                if left_o is not None and right_o is not None:
                    last_left, last_right = left_o, right_o
                    last_left_proc, last_right_proc = left_p, right_p
        ...
        left_roi_proc = last_left_proc if roi_valid else None
        right_roi_proc = last_right_proc if roi_valid else None

        # Feature extraction is intentionally on original grayscale.
        lf = feature_one_eye(orig_gray, left_roi, args)
        rf = feature_one_eye(orig_gray, right_roi, args)
        ...
        row = {
            ...
            # Original-frame ROIs. These are the coordinates to feed to PL.
            ...
            # Processed-frame ROIs from dlib, for debugging mapping quality.
            "proc_left_x": left_roi_proc[0] if left_roi_proc else -1,
            "proc_left_y": left_roi_proc[1] if left_roi_proc else -1,
            "proc_left_w": left_roi_proc[2] if left_roi_proc else -1,
            "proc_left_h": left_roi_proc[3] if left_roi_proc else -1,
            "proc_right_x": right_roi_proc[0] if right_roi_proc else -1,
            "proc_right_y": right_roi_proc[1] if right_roi_proc else -1,
            "proc_right_w": right_roi_proc[2] if right_roi_proc else -1,
            "proc_right_h": right_roi_proc[3] if right_roi_proc else -1,
        }
        row.update(feats)
        rows.append(row)
    cap.release()
...
def feature_columns(df: pd.DataFrame) -> List[str]:
    skip = {"video_id","frame_idx","time_sec","orig_frame_w","orig_frame_h","proc_frame_w","proc_frame_h","proc_scale_x","proc_scale_y","label_closed","face_ok_on_update","roi_updated","roi_valid","roi_age","left_x","left_y","left_w","left_h","right_x","right_y","right_w","right_h","proc_left_x","proc_left_y","proc_left_w","proc_left_h","proc_right_x","proc_right_y","proc_right_w","proc_right_h"}
...
def main():
    ...
    parser.add_argument("--out_dir", default=r"C:\Users\LC\Desktop\eyeblink8\work_selfrec_candidates_hybrid_half_lowfreq")
    parser.add_argument("--proc_scale", type=float, default=0.5, help="Resize whole frame before dlib only. 0.5 means dlib sees half width and half height; EyeFeature still uses the original frame.")
    ...
    parser.add_argument("--roi_mode", choices=["each_frame","lowfreq"], default="lowfreq")
    parser.add_argument("--detect_interval", type=int, default=20)
    parser.add_argument("--reuse_max_age", type=int, default=30)
    parser.add_argument("--detect_width", type=int, default=0, help="Optional extra resize width inside dlib detection after --proc_scale. Default 0 means no second resize.")
    ...
    print(f"[Info] dlib proc_scale={args.proc_scale}, proc_width={args.proc_width}, proc_height={args.proc_height}, interpolation={args.resize_interpolation}")
    print("[Info] EyeFeature is computed on original-resolution grayscale using mapped-back ROIs")
