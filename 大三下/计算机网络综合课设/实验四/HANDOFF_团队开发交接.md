...
- AutoDL 部署时客户端连接的是公网 IP，不是 `127.0.0.1`。


## AutoDL 服务器对接说明

当前 1 号已完成 AutoDL 基础环境配置：

- 系统：Ubuntu 22.04
- Java：OpenJDK 17 已安装
- 项目目录：`/root/ch04`
- 服务端数据目录：`/root/ch04/chat-data`
- 启动脚本：`/root/ch04/deploy/run-server.sh`
- 停止脚本：`/root/ch04/deploy/stop-server.sh`
- 服务端内部监听端口：`3500`
